...
public class WakeWordHelper
{
    ...
    {
        if (_isPaused || _disposed || session == null)
            return;
        ...
        {
            Debug.WriteLine($"Session was already disposed: {ex.Message}");
        }
        catch (Exception ex)
        {
            Debug.WriteLine($"Error running ONNX model: {ex.Message}");
        }
    }
    ...
    public void Stop()
    {
        ...
        Debug.WriteLine("WakeWordHelper stopped.");
    }
}
